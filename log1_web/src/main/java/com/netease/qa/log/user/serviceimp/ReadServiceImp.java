...
//private  UkExceptionDataService ukExceptionDataService = new UkExceptionDataService();	  
//   private   ExceptionDataService exceptionDataService ;
     @Resource
	...
	public JSONObject queryTimeRecords(int logSourceId, long startTime,
        ...
        //System.out.println("first.getExceptionId():" + first.getExceptionId());
		...
		for(int i = 1; i < exceptionDatas.size(); i++){
			...
			if(convert(next.getSampleTime()).equals(record.getString("time"))){
				...
				detail.put(this.exceptionDao.findByExceptionId(next.getExceptionId()).getExceptionType(), next.getExceptionCount());
				details.add(detail);
			}
			else{
	...
	public JSONObject queryErrorRecords(int logSourceId, long startTime,
		...
		for(int i = 1; i < exceptionDatas.size(); i++){
			...
			if(this.exceptionDao.findByExceptionId(next.getExceptionId()).getExceptionType().equals(error.getString("type"))){
