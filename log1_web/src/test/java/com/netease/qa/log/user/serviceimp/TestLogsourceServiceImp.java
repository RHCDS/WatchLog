...
import com.netease.qa.log.user.service.impl.LogsourceServiceImp;
...
public class TestLogsourceServiceImp {
	...
	public void testAddLogsource() {
		...
		int logsource = logsourceService.addLogsource("log", 2, "localhost://qg", "test1/helloworld1",
		...
		// 插入已存在的日志源
		int logsource1 = logsourceService.addLogsource("log4", 4, "localhost://", "test1/helloworld1",
				"helloWorld1.txt", "\\d{4}\\-\\d{2}\\-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}.\\d{3}", "error",
				"java.net.NoRouteToHostException: this is a NoRouteToHostException", "lilei");
		Assert.assertEquals("插入已存在的日资源，竟然插入成功了！", -2, logsource1);

		// 插入不存在的项目
		int logsource2 = logsourceService.addLogsource("log4", 900, "localhost://qgq", "test1/helloworld1",
				"helloWorld1.txt", "\\d{4}\\-\\d{2}\\-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}.\\d{3}", "error",
				"java.net.NoRouteToHostException: this is a NoRouteToHostException", "lilei");
